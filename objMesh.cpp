...
int Parse(std::vector<Token>& tokens, ObjMesh* mesh)
{
    ...
    while(parseToken)
    {
        ...
        case OBJ_TOK_VERTEX:
            {
                Vector3 v;
                if(ParseVertex(parseToken, &nextToken, &v) ) return 1;
        ...
        case OBJ_TOK_VERTEX_NORMAL:
            {
                Vector3 vn;
                if(ParseVertex(parseToken, &nextToken, &vn) ) return 1;
        ...
        case OBJ_TOK_TEXCOORD:
            {
                Vector2 xtexcoord;
                if(ParseTexcoord(parseToken, &nextToken, &xtexcoord)) return 1;
        ...
        case OBJ_TOK_FACE:
            {
                ObjMesh::Face poly;
                if(ParseFaceVertex(parseToken, &nextToken, &poly)) return 1;
...
bool LoadObj(const char* path, ObjMesh* mesh)
{
    ...
    bool success = LoadObj(strBuf, fileLen, mesh);
